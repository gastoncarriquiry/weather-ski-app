...
arrayCiudades.forEach(destino => {
    ...
    destino1.insertAdjacentHTML("beforeend", `<option value="${valueDestino}">${localidadDestino}, ${paisDestino}</option>`);
    destino2.insertAdjacentHTML("beforeend", `<option value="${valueDestino}">${localidadDestino}, ${paisDestino}</option>`);
});
...
const obtenerUbicacion = () => {
    ...
    else if (ubicDestino1 != "") { destino1.style.border = "1px solid transparent"; }
    else if (ubicDestino2 != "") { destino2.style.border = "1px solid transparent"; }
}
...
const buscarPronostico1 = () => {
    ...
    if (ubicDestino1 != "" && ubicDestino2 != "") {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat1}&lon=${lon1}&exclude=current,minutely,hourly,alerts&appid=7c4d35545efb8245cdb4bccb37bf97c0&lang=es&units=metric`)
            .then(r => r.json())
            .then(infoProno1 => {
                ...
                if (infoProno1.cod != "404") {
                    ...
                    document.querySelector("#cardDestino1").insertAdjacentHTML("afterbegin", `
                    ...
                    infoProno1.daily.forEach(pronostico => {
                        document.querySelector("#cont-diarios1").insertAdjacentHTML("beforeend", `
                        <article class="pronoDia">
                            <div>
                                <h2>${definirDia(pronostico.dt)}</h2>
                                <img src="img/${pronostico.weather[0].icon}.png" alt="${pronostico.weather[0].main}" class="icon">
                                <div>
                            ...
                            <div>
                                <p>Vel. del viento: ${redondearDecimales(pronostico.wind_speed, 1)} m/s</p>
                ...
                else { document.querySelector("#error404").style.display = "inline-block"; }
            })
    }
    else if (ubicDestino1 != "") { destino1.style.border = "1px solid transparent"; }
    else { destino1.style.border = "1px solid red"; }
}
...
const buscarPronostico2 = () => {
    ...
    if (ubicDestino2 != "" && ubicDestino1 != "") {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat2}&lon=${lon2}&exclude=current,minutely,hourly,alerts&appid=7c4d35545efb8245cdb4bccb37bf97c0&lang=es&units=metric`)
            .then(r => r.json())
            .then(infoProno2 => {
                document.querySelector("#loader").style.display = "none";
                ...
                if (infoProno2.cod != "404") {
                    ...
                    document.querySelector("#cardDestino2").insertAdjacentHTML("afterbegin", `
                        <img src="img/imgLugares/${levantarCiudad[0].src}" alt="${levantarCiudad[0].alt}" class="imgDestino">
                        <h1 class="tituloDestino">${levantarCiudad[0].localidad}, ${levantarCiudad[0].pais}</h1>`
                    );

                    infoProno2.daily.forEach(pronostico => {
                        document.querySelector("#cont-diarios2").insertAdjacentHTML("beforeend", `
                        <article class="pronoDia">
                            <div>
                                <h2>${definirDia(pronostico.dt)}</h2>
                                <img src="img/${pronostico.weather[0].icon}.png" alt="${pronostico.weather[0].main}" class="icon">
    ...
    else { destino2.style.border = "1px solid red"; }
}
...
const compararPronosticos = () => {
    ...
    if (promedioPrecipitaciones1 > promedioPrecipitaciones2) {
        puntos1++;
        document.querySelector("#param-precip").insertAdjacentHTML("afterbegin", `
    ...
    if (promedioTemp1 < promedioTemp2) {
        puntos1++;
        document.querySelector("#param-temp").insertAdjacentHTML("afterbegin", `
    ...
    if (promedioViento1 < promedioViento2) {
        puntos1++;
        document.querySelector("#param-viento").insertAdjacentHTML("afterbegin", `
    ...
    if (puntos1 > puntos2) {
        document.querySelector("#resultadoComparacion").insertAdjacentHTML("afterbegin", `
